...
/*
 * To measure the dutycycle and the frequency
*/
void PWM_Measure(u32* Pfreq,u8* Pduty)
{
	...
	TCNT1=0;
	Timer1_ICU_SetCallBack(Func_ICU); // Capture edges times
	Timer1_InputCaptureEdge(RISING);  // Capture Edge config
	Timer1_ICU_InterruptEnable();	  // Enable interrupt when u capture an edge
	flag=0;
	while(flag<3); // W8 till u capture all the edges times (t1, t2, t3)
	Ton=t2-t1;
	Toff=t3-t2;
...
/*
 * The function that captures the rising, falling, second rising edges times
*/
...
{
	Timer1_Init(TIMER1_CTC_OCRA_TOP_MODE,TIMER1_SCALER_8, OCRA_INVERTING, OCRB_NON_INVERTING);
	OCR1A=999;
